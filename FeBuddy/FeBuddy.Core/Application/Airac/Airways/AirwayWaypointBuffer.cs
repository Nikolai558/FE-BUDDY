		...
		string awyId)
	{
		...
		foreach (LineString lineString in lineStrings)
		{
			...
			for (int i = 0; i < coordinates.Length - 1; i++)
			{
				...
				double startToEndBearing = GeoMath.Bearing(startLocation, endLocation);
				double endToStartBearing = GeoMath.Bearing(endLocation, startLocation);

				Location bufferedStart =
					GeoMath.PointAtDistanceAndBearing(startLocation, startToEndBearing, startRadius);
