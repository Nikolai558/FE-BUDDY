...
public static class SctToGeojsonService
{
	...
	public static SctToGeojsonServiceResult Run(
		...
		IProgress<ConversionProgress>? progress = null)
	{
		...
		IReadOnlyList<SctFileConversion> conversions = ConversionFiles.ConvertEach(
			sources,
			source => Convert(source, parsed, files, messages),
			(source, error) => new SctFileConversion { SourcePath = source, Error = error },
			Describe,
			progress,
			LogSource,
			messages);
		...
		return new SctToGeojsonServiceResult
		{
			Messages = messages,
	...
	private static SctFileConversion Convert(
		string source,
	...
	private static string Describe(SctFileConversion conversion) => conversion switch
	{
		{ Error: { } error } => error,
