...
.SYNOPSIS
    ...
    See docs/Developers/MSI-VERSION-NUMBERING.md for the full explanation of why this exists.
...
.DESCRIPTION
    ...
    "3.0.0-alpha.1"). This script derives a disposable, order-agnostic internal number:
    ...
    (FeBuddy.Installer.CustomActions) does that using the real semantic version instead.
...
.PARAMETER RealVersion
    The real FE-BUDDY semantic version being built, e.g. "3.0.0-alpha.1" or "3.0.0".
...
if ($null -eq $counter -or [int]$counter.major -ne $realMajor) {
    ...
    if ($nextMinor -gt 255) {
        throw ("Get-InstallerVersion: counter exhausted for major version $realMajor " +
               ...
               "see docs/Developers/MSI-VERSION-NUMBERING.md).")
    }
