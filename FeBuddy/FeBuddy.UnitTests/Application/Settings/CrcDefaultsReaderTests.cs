...
public class CrcDefaultsReaderTests
{
	...
	[Theory]
	[InlineData(CrcFeatureKind.Line)]
	[InlineData(CrcFeatureKind.Symbol)]
	[InlineData(CrcFeatureKind.Text)]
	public void read_include_is_false_when_the_flag_is_absent(CrcFeatureKind kind)
	{
		Assert.False(CrcDefaultsReader.ReadInclude(new Dictionary<string, string>(), kind));
	}

	[Theory]
	[InlineData(CrcFeatureKind.Line, "IncludeCrcLineDefaults", "Y", true)]
	[InlineData(CrcFeatureKind.Line, "IncludeCrcLineDefaults", "N", false)]
	[InlineData(CrcFeatureKind.Symbol, "IncludeCrcSymbolDefaults", "y", true)]
	[InlineData(CrcFeatureKind.Symbol, "IncludeCrcSymbolDefaults", "N", false)]
	[InlineData(CrcFeatureKind.Text, "IncludeCrcTextDefaults", "Y", true)]
	[InlineData(CrcFeatureKind.Text, "IncludeCrcTextDefaults", "n", false)]
	public void read_include_reads_its_kinds_flag(CrcFeatureKind kind, string key, string value, bool expected)
	{
		Dictionary<string, string> settings = new() { { key, value } };

		Assert.Equal(expected, CrcDefaultsReader.ReadInclude(settings, kind));
	}

	[Theory]
	[InlineData(CrcFeatureKind.Line, "IncludeCrcLineDefaults")]
	[InlineData(CrcFeatureKind.Symbol, "IncludeCrcSymbolDefaults")]
	[InlineData(CrcFeatureKind.Text, "IncludeCrcTextDefaults")]
	public void read_include_throws_naming_the_key_when_the_flag_is_not_y_or_n(CrcFeatureKind kind, string key)
	{
		Dictionary<string, string> settings = new() { { key, "maybe" } };

		ArgumentException ex = Assert.Throws<ArgumentException>(() => CrcDefaultsReader.ReadInclude(settings, kind));

		Assert.Contains(key, ex.Message);
	}

	[Fact]
	public void read_include_reads_each_kind_independently()
	{
		Dictionary<string, string> settings = new()
		{
			{ CrcDefaultsReader.IncludeLineKey, "N" },
			{ CrcDefaultsReader.IncludeSymbolKey, "Y" }
		};

		Assert.False(CrcDefaultsReader.ReadInclude(settings, CrcFeatureKind.Line));
		Assert.True(CrcDefaultsReader.ReadInclude(settings, CrcFeatureKind.Symbol));
		Assert.False(CrcDefaultsReader.ReadInclude(settings, CrcFeatureKind.Text));
	}
}
