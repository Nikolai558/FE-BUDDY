...
namespace FeBuddy.UnitTests.Domain.Crc;
...
public class CrcPropertyValidatorTests
{
	...
	{
		Assert.Equal(expectedValid, CrcPropertyValidator.ValidateLine(Line(bcg: bcg)).IsValid);
	}

	[Fact]
	public void bcg_null_is_valid_because_crc_auto_assigns_it()
	{
		Assert.True(CrcPropertyValidator.ValidateLine(Line(bcg: null)).IsValid);
	}
	...
	{
		Assert.Equal(expectedValid, CrcPropertyValidator.ValidateLine(Line(style: style)).IsValid);
	}
	...
	{
		Assert.Equal(expectedValid, CrcPropertyValidator.ValidateLine(Line(thickness: thickness)).IsValid);
	}
	...
	{
		Assert.Equal(expectedValid, CrcPropertyValidator.ValidateSymbol(Symbol(style: style)).IsValid);
	}
	...
	{
		Assert.Equal(expectedValid, CrcPropertyValidator.ValidateSymbol(Symbol(size: size)).IsValid);
	}
	...
	{
		Assert.Equal(expectedValid, CrcPropertyValidator.ValidateText(Text(size: size)).IsValid);
	}
	...
	{
		Assert.True(CrcPropertyValidator.ValidateText(Text(xOffset: xOffset)).IsValid);
	}
	...
	{
		Assert.True(CrcPropertyValidator.ValidateText(Text(yOffset: yOffset)).IsValid);
	}
