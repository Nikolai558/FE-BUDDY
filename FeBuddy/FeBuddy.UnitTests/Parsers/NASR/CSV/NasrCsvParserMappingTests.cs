...
namespace FeBuddy.UnitTests.Parsers.NASR.CSV;
...
public sealed class NasrCsvParserMappingTests
{
	...
	/// <summary>
	/// <see cref="NasrCsvParserController.MainAsync"/> parses every group from one directory and
	/// fills every list of the combined collection.
	/// </summary>
	[Fact]
	public async Task MainAsync_ParsesEveryFileIntoTheCombinedCollection()
	{
		NasrCsvDataCollection all = await NasrCsvParserController.MainAsync(new[] { FixtureDirectory });
	...
	public async Task MainAsync_MissingDirectory_Throws()
	{
		...
		await Assert.ThrowsAnyAsync<IOException>(() => NasrCsvParserController.MainAsync(new[] { missing }));
	}
	...
	/// <summary>What the parser should produce for a raw CSV value, mirroring <see cref="FebCsvHelper"/>.</summary>
