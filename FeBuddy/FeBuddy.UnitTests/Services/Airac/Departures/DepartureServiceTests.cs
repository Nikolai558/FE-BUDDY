...
public sealed class DepartureServiceTests : IDisposable
{
	...
	public void a_single_point_procedure_gets_symbols_and_text_but_no_lines_file()
	{
		...
		DepartureGeojsonGenerateResult result = DepartureGeojsonService.Generate(new[] { airportProcedure }, settings);
	...
	public void geojson_generate_rejects_null_arguments()
	{
		...
		Assert.Throws<ArgumentNullException>(() => DepartureGeojsonService.Generate(null!, settings));
		Assert.Throws<ArgumentNullException>(() => DepartureGeojsonService.Generate(Array.Empty<DepartureAirportProcedure>(), null!));
	}
	...
	{
		Assert.Equal(expected, DepartureGeojsonService.Name(property));
	}
	...
	public void alias_skips_a_procedure_with_no_points_and_warns_on_a_duplicate_command()
	{
		...
		DepartureAliasGenerateResult result = DepartureAliasService.Generate(
			new[]
			{
	...
	public void alias_with_nothing_to_write_writes_no_file()
	{
		...
		Assert.Throws<ArgumentNullException>(() => DepartureAliasService.Generate(null!, settings));
		Assert.Throws<ArgumentNullException>(() => DepartureAliasService.Generate(Array.Empty<DepartureAirportProcedure>(), null!));
	}
