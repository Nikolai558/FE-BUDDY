	...
	IOutputSettings, IFebPropertySettings, ICrcDefaultsSettings, IRoiOverrideSettings
{
	...
	public override IReadOnlyList<ServiceReviewSection> BuildReviewSummary()
	{
		...
		string[] selectedFebProperties = [.. FebProperties.Where(p => p.IsSelected).Select(p => p.Name)];
		...
		return [new ServiceReviewSection("Airways", rows)];
	}
	...
	{
		ObservableCollection<EramClassDefault> rows = [];
