...
| `SelectedSubServices` | comma-separated keys: `Airports`, `Airways`, `Departures`, `Arrivals`, `Navaids`, `ArtccBoundaries` | none | General tab. Keys are stable identifiers - never rename one without migrating this value. |
...
| Key | Values | Default |
|---|---|---|
| `GenerateAliasFile` | `Y` / `N` - not saved for ARTCC Boundaries, which has no alias file | `Y` |
| `EmitLines`, `EmitSymbols`, `EmitText` (Airports: `EmitRunwayLines`, `EmitAirportSymbols`, `EmitAirportText`; NAVAIDs: `EmitSymbols`, `EmitText` only - there is no `EmitLines`; ARTCC Boundaries: none of these - it always writes Lines only, with no choice) | `Y` / `N` | `Y` |
